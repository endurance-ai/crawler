...
- **ZARA**: Akamai Bot Manager active deployment, no public API, ToS unverified. Deferred to follow-up `SPEC-PLATFORM-002` whose entry condition is the 7-day Uniqlo soak window described in plan.md §2.4. The user has indicated willingness to attempt a Playwright-based engine for ZARA in that follow-up SPEC; that decision is captured but not implemented here.
- **29CM**: Robots.txt is permissive but the Next.js App Router architecture means there is no public JSON API; data ships via React Server Components payloads requiring either Playwright or RSC reverse-engineering. Deferred to follow-up `SPEC-PLATFORM-003` under the same 7-day soak entry condition.

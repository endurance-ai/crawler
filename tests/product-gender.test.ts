...
test("명시적 모델 라벨은 한쪽/양쪽 착용을 구분하고 일반 문장은 무시한다", () => {
  ...
  assert.equal(inferGenderFromModelDescription("깔끔한 디자인의 여성용 티셔츠입니다."), "women")
  assert.equal(inferGenderFromModelDescription("일상에서 편하게 입는 남성용 셔츠입니다."), "men")
  assert.equal(inferGenderFromModelDescription("모델 M 착용 (185cm)"), null)
  ...
  assert.equal(inferGenderFromModelDescription("* 여자 모델 S 착용 (176cm)"), "women")
  assert.equal(inferGenderFromModelDescription("*남자모델 M착용 (180cm)"), "men")
  assert.equal(inferGenderFromModelDescription("여자모델 S 착용 / 남자모델 M 착용"), "unisex")
  assert.equal(inferGenderFromModelDescription("우먼스 라인으로 출시되었습니다."), "women")
  assert.equal(inferGenderFromModelDescription("세미 오버핏 유니 티셔츠입니다."), "unisex")
  assert.equal(inferGenderFromModelDescription("누구나 착용해도 부담스럽지 않은 핏"), "unisex")

...
- **리스트에 없는 상품**: 완전성 가드(`--min-coverage`, 기본 0.7) 를 통과한 런에서만
  ...
  임계값을 낮출 때는 근거가 필요하다.
- **연속 실패 소스는 쉬게 한다 (서킷브레이커)**: `refresh-listing` 이 시작할 때
  `product_refresh_runs` 이력에서 마지막 성공 이후 연속 실패 횟수를 세고, 3회부터
  계단식으로 backoff 한다 — 3회 12시간, 4회 1일, 5회 3일, 6회 7일, 7회 이상 14일.
  2회까지는 쉬지 않는다(일시 장애를 영구 격리하지 않기 위해). 억제된 소스는 시작
  로그에 `🔌 서킷브레이커 대기 N개` 로 항상 표시되므로 소스가 조용히 사라지지 않는다.
  강제 실행은 `--ignore-backoff`.
  근거(실측 2026-07-29): 전체 런 76.8시간 중 연속 3회 이상 실패 중인 소스가
  5.8시간을 태우고 있었고, 그 대부분이 이 호스트에서 아예 닿지 않는 해외 사이트
  (kith, browns, zara-kr, zara-us, end)였다. 재시도해도 영원히 실패한다.
  → 항구적으로 못 닿는 소스는 backoff 에 맡기지 말고 `REFRESH_EXCLUDE` 에 넣는 것이 맞다.

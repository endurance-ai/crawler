// 휴면 — 모델컷 선별용 Apple Vision 분류기. **macOS 15+ 전용.**
// 배선 상태·재활성화 제약은 ../../src/select-product-images.ts 헤더 참조.
// 수동 컴파일 불필요 — product-image-vision-client.ts 가 xcrun clang 으로
// mtime 비교 후 자동 빌드한다 (캐시 디렉터리에 바이너리 배치).

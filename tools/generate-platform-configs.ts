...
const SIXSHOP_CATEGORY_URLS_BY_KEY: Partial<Record<string, string[]>> = {
  // Sixshop Storefront v2 keeps the shop link inside server component data,
  // not a rendered home-page anchor. Pin the official catalogue landing page.
  findoubt: ["https://findoubt.com/shop"],
}
...
interface GeneratedCandidate extends Omit<CandidateRow, "platform_key" | "platform_type"> {
  platform_key: string
  platform_type: Extract<PlatformType, "shopify" | "cafe24" | "imweb" | "sixshop">
}
...
async function fetchCandidates(): Promise<GeneratedCandidate[]> {
  ...
  for (const raw of (data ?? []) as CandidateRow[]) {
    ...
    if (
      ...
      (type !== "cafe24" && type !== "shopify" && type !== "imweb" && type !== "sixshop")
    ) {
      continue
    }
...
function buildEntrySource(
  ...
  if (row.platform_type === "cafe24") {
    ...
    lines.push("    crawlDetails: true,")
  } else if (row.platform_type === "sixshop") {
    const productPath = row.platform_key === "en3ai" || row.platform_key === "yinandyang" ||
        row.platform_key === "hyxia" || row.platform_key === "suhwasystem" ||
        row.platform_key === "findoubt"
      ? "products"
      : "product"
    lines.push(`    sixshopProductPath: ${JSON.stringify(productPath)},`)
    const categoryUrls = SIXSHOP_CATEGORY_URLS_BY_KEY[row.platform_key]
    if (categoryUrls?.length) lines.push(`    categoryUrls: ${JSON.stringify(categoryUrls)},`)
    lines.push("    sourceCurrency: \"KRW\",")
    lines.push("    maxPages: 100,")
    lines.push("    crawlDetails: false,")
  }
 ...
 *
 * Auto-generated shopify/cafe24/imweb/sixshop sources without a manual config.

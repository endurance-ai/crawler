...
// 크롤 결과를 product_crawl_status(090, brand_node_id 기준)에 자동 반영한다(수기 mark 불필요).
...
async function crawlCafe24WithChromium(
  ...
  try {
    ...
    return await crawlCafe24(page, config, detailParser, reviewParser, options)
  } finally {
    await browser.close()
  }
}
...
async function crawlCafe24WithSelectedEngine(
  ...
  const options = {detailConcurrency, onDetailProgress, existingDetails}
  if (mode === "chromium") {
    return await crawlCafe24WithChromium(config, detailParser, reviewParser, options)
  }

 ...
 *
 * 배경: 크롤러가 상품에서 성별을 못 뽑으면 brand_nodes.gender_scope 를 그대로
 * 복사했고(런북 092 의 백필 13,942행 포함), 스코프가 ['unisex'] 인 브랜드에서는
 * "모름"이 "unisex" 로 세탁됐다. 검색 RPC 는 `p.gender && ARRAY[p_gender,'unisex']`
 * 라 unisex 상품을 남녀 양쪽에 노출하므로, 그 상품들이 남성 결과에 여성복으로
 * 떠 있다.
 *
...
export function classifyGenderRepair(
  ...
  brandScopeById: Map<number, ProductGender[]>,
  opts: {useDescription?: boolean} = {},
): GenderRepairDecision {
  const before = cleanGenderScope(row.gender)
  const brandScope = row.brand_node_id !== null ? brandScopeById.get(row.brand_node_id) : undefined

  const resolved = resolveProductGenderWithSource([], brandScope, {
    name: row.name,
...
function repairSourceOf(source: GenderSource | null): GenderSource | null {
  ...
  if (source === "brand_scope") return "repair_brand_scope"
  return source
}

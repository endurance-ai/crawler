 ...
 * 후보가 통째로 brand_unmatched 로 파킹됐다. 실측(파킹 44,466건 중):
 *   - brand-4cb9: "the pink" 865건은 정상, 나머지 1,866건이 서로 다른 **상품명**
 *   - glowny:     1,345건 전부 brand = "ㅤ"(U+3164 한글 채움문자)
 *   - lowclassic: 638종이 "Oversized Lc Checked ShirtBlue" 류 상품명+색상
 * 이들은 전부 `multiBrand` 미표시 + `config.brand` 설정된 자사몰이다.
 */

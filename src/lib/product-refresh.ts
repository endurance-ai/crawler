...
export async function loadExistingBrands(
  ...
  const pageSize = 1000
  for (let offset = 0; ; offset += pageSize) {
    // ORDER BY 없는 LIMIT/OFFSET 은 페이지 간 행 순서가 보장되지 않아 중복·누락이
    // 생길 수 있다 (Postgres 가 plan 을 바꾸면 바로 발현). 브랜드 하나가 누락되면
    // 그 브랜드 신규 상품이 통째로 brand_unmatched 로 파킹되므로 안정 정렬 필수.
    const {data, error} = await db
      .from("brand_nodes")

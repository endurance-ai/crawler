...
export interface CrawlCafe24Options {
  ...
  onDetailProgress?: (products: Product[]) => void | Promise<void>
      ...
      context?: () => {browser?: () => {
        newContext: () => Promise<{
          ...
          newPage: () => Promise<Cafe24Page & {
            on?: (event: "dialog", handler: (dialog: {dismiss: () => Promise<unknown>}) => void) => unknown
          }>
...
export async function crawlCafe24(
  ...
  if (config.crawlDetails && detailParser) {
    ...
    const createDetailPage = options.createDetailPage ?? createPlaywrightDetailPageFactory(page)
    ...
    // 상품마다 context/page를 만들고 닫지 않고, worker lease를 상세 크롤 동안
    // 재사용한다. Chromium은 Playwright context를, Lightpanda는 pool lease를 쓴다.
    const workerLeases: Cafe24DetailPageLease[] = []
    for (let w = 0; w < DETAIL_CONCURRENCY; w++) {
      workerLeases.push(await createDetailPage())
    }
    ...
    try {
        ...
        const results = await Promise.all(
          batch.map(async (product, slot) => {
            ...
            try {
              ...
              // 로드된 detail page에서 옵션/스와치/스펙표도 범용 스캔한다.
              if (!detail.color) {
      ...
      for (const lease of workerLeases) {

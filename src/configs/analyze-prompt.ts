...
=== RULES ===
- CRITICAL — PRODUCT IDENTIFICATION: You may receive product name, shop category, material, and description as hints.
...
export function buildProductAnalyzeUser(hint?: {
  ...
  if (hint?.name || hint?.category || hint?.description || hint?.material) {
    ...
    parts.push("Use these hints along with the image to accurately classify this product. Trust the text data for fabric/material when available — the image confirms color, silhouette, and fit.")
    return parts.join("\n")
  }
